...
from quiz import generate_daily_questions, generate_retry_questions, get_math_retry_from_bank
...
def main():
    ...
    # 数学のみ再挑戦問題をバンクから別の問題として取得。他の科目は同じ問題を再出題。
    retry_map = {}
    math_qs = [(i, q) for i, q in enumerate(questions) if q.get('subject') == '数学']
    logger.info(f'数学の再挑戦問題をバンクから取得: {len(math_qs)}問')
    for idx, q in math_qs:
        retry_q = get_math_retry_from_bank(q, settings)
        if retry_q:
            retry_map[idx] = retry_q
            logger.info(f'  数学再挑戦 問{idx + 1}: バンクから取得')
        else:
            logger.info(f'  数学再挑戦 問{idx + 1}: バンクに問題なし（同問題を使用）')
